...
* **Core Architecture (DOTS 1.0+):** * `Unity.Entities` (Data-Oriented ECS Framework)
  ...
  * `Unity.Mathematics` (SIMD-friendly math library for custom collisions)
* **Frameworks & Packages:**
  * **Input:** `Input System Package (New)` (Event-driven input handling)
  * **UI:** `UGUI (Canvas)` (Managed-Unmanaged Hybrid UI Bridge)
